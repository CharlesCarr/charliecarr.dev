...
_This blog post is the first in the series that follows along with my github repo ([link](https://github.com/CharlesCarr/grand-slams-dashboard)) and the live app is hosted on Vercel ([link](https://grand-slams-dashboard.vercel.app/))._
...
- [GitHub Repository](https://github.com/CharlesCarr/grand-slams-dashboard)
- [Live Application](https://grand-slams-dashboard.vercel.app/)
...
In late 2022, Vercel released Next 13 bringing on major changes to support the new React 18 paradigm (client / server components) and most notably the introduction to the app router that was recently declared stable by the team this May.
...
As I started mapping out this application, I came across [shadcn/ui](https://ui.shadcn.com/) re-usable components and his example dashboard. I found an interesting dataset on tennis grandslam finals results and quickly spun up a Supabase Postgres DB with tables to serve the data for my own dashboard to display this data in interesting ways for tennis fans.
...
This concept is foundational to the App Router (and probably my favorite aspect). It allows developers to house relevant files – be it components, styles, tests, or more – within these route folders. This was not possible in the pages directory as any file in pages is considered a route. Now, only the 'page' files are publicly accessible, ensuring a neat and organized structure.
...
The new Next.js 13 features have been a great developer experience for me so far and I am looking forward to building and sharing more in my next article in the series. Stay tuned.
